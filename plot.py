...
def main():
    ...
    for i in range(100, 500):
        theta_vector, C, r, dr = whole_BA(i, d_threshold = 0.0002, d_step= 1e-8, use_cuda=args.use_cuda)
        print("[+]       {}              {}      ".format(i, len(theta_vector)))

        input_x_output = '{}x{}_'.format(len(theta_vector), i)
        file_name = time.strftime("%H-%M-%S_%d-%m-%Y.pt", t)
        file_obj = {'theta': theta_vector, 'r': r, 'C': C, 'dr': dr}
        path = './outputs/classic/data/' #r'D:/User/University/MSc/final_project/files/'
        tr.save(file_obj, path+input_x_output+file_name)

    exit()

    loader = tr.load("./outputs/classic/data/22-32-31_12-02-2023.pt")
    theta_vector = loader["theta"]
    r = loader["r"]
    C = loader["C"]

    # PC.plot_r(theta_vector, output_size, r)
    PC.plot_r_and_C(theta_vector, output_size, r, C)
